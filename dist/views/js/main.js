...
// Add an event listener to take this out of the DOM
document.getElementById("sizeSlider").addEventListener("change", function(e) {
    // Trigger the resizing of the pizzas
    resizePizzas(e.target.value);
});
...
function updatePositions() {
  ...
  // Small optimization (could be considered micro) is using a faster selector
  var pizzas = document.getElementsByClassName("mover");

  // We can precalculate the phases here and save on the performance of the
  // the Math.sin
  for (var j = 0; j < 5; j++) {
  ...
  for (var k = 0; k < pizzas.length; k++) {
    ...
    pizzas[k].style.transform = "translate3d(" + distance + "px, 0, 0)";
  }
...
// runs updatePositions on scroll - We are gonna update the positiion
// on a more optimized path. Instead of binding the updates/animations to
// the scroll event, we're gonna try requestAnimationFrame which is more convenient
// for the browser and smooth out the animation on the screen. We request a frame to
// draw in only if we are scrolling.
window.addEventListener("scroll", requestFrame, false);

// the requestFrame function allows the application to render when
// convenient for the browser. If the app is rendering, then the
// request is is not granted (i.e, no animation frame is used)
...
// Boolean variable here to help determine if we are indeed in a animation frame update
// or not (OPT)
var isInFrame = false;
var lastY = 0;
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  // Removed the the loop querying, 200 times to find the #movingPizzas1
  ...
  // Update the position of created pizzas
  requestFrame();
});
